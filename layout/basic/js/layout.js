window.addEventListener('load', function(){
    ...
    //quickGoTop(); 사용안함 
    searchLayer();
    //toggleClass('.xans-layout-info.info__customer', '.xans-layout-info.info__customer .toggle', 'selected'); 사용안함 
    //topBanner(); 사용안함 
	handleScroll();
});
...
function handleScroll(){
	...
	//setQuickScrollEvent(scrollY, quickMenu); 사용안함 
	window.addEventListener('scroll', function(e) {
        scrollPosition = window.scrollY || window.pageYOffset;
        if (ticking) return;
        window.requestAnimationFrame(function() {
            fixedHeader()
            //setQuickScrollEvent(scrollPosition, quickMenu) 사용안함 
            ticking = false;
        });
        ticking = true;
	});
...
jQuery(document).ready(function() {
	...
	if (jQuery('.xans-member-login').val() != undefined) {
        jQuery('.login_page_area #member_id').attr('placeholder', '아이디');
		jQuery('.login_page_area #member_passwd').attr('placeholder', '비밀번호');
	}

	/* 비회원 주문조회페이지 placeholder 추가 */
	setTimeout(function(){
		if (jQuery('.xans-myshop-orderhistorynologin').val() != undefined) {
			jQuery('.login_page_area #order_name').attr('placeholder', '주문자명');
			jQuery('.login_page_area #order_id').attr('placeholder', '주문번호(하이픈(-) 포함)');
			jQuery('.login_page_area #order_password').attr('placeholder', '비회원주문 비밀번호');
		}
	}, 100);
	...
	jQuery(".xans-member-login .login__sns .wrap_sns_log a").each(function(){
		...
		if( wrap_sns_log == false){
            ...
            jQuery("#login_area").css('margin-top','100px');
		}
	});
